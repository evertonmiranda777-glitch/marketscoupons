...
async function getLivePromoBlock() {
  ...
  try {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/cms_firms?active=eq.true&select=id,short_name,name,coupon,discount,discount_type,promo_ends_at,promo_label,disc_note,prices`, {
      headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
    });
    if (!r.ok) return _promoCache.text || '';
    const firms = await r.json();
    // FONTE UNICA do dado de afiliado: tabela `firms`. Sobrescreve o cupom que veio do cms_firms,
    // pra o Max nunca oferecer codigo velho (incidente 27/jul: E8 estava com 'E8' em vez de MARKET).
    try {
      const ra = await fetch(`${SUPABASE_URL}/rest/v1/firms?select=slug,coupon_code,affiliate_url&ativo=eq.true`, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
      });
      if (ra.ok) {
        const aff = await ra.json();
        const m = {}; aff.forEach((x) => { m[x.slug] = x; });
        for (const f of firms) { const a = m[f.id]; if (a) { f.coupon = a.coupon_code || null; f.affiliate_url = a.affiliate_url; } }
        _cupomPorSlug = m;   // usado para resolver {{CUP:slug}} no prompt
      }
    } catch (_) {}
    const lines = [];
    const priceLines = [];
...
const LANG_NAMES = {
  ...
  de: 'German',
  ar: 'Arabic',
  id: 'Indonesian (Bahasa Indonesia)',   // o site tem 8 idiomas; faltava este
};
...
- If the trader mentions a code they saw on the firm's OWN site: give ours and move on. NEVER name the other code and NEVER tell them to "compare at checkout to see which is better" - a public code pays ZERO commission, and the commission is what keeps this site and its free tools alive. The only exception is CITY TRADERS IMPERIUM, and it is written in the CTI entry below.
...
- ALWAYS actively promote the coupon: "Se voce usar o cupom {{CUP:apex}}, ganha 90% de desconto" / "Use code {{CUP:bulenox}} for 89% off", never just mention the code passively.
...
- After recommending a firm, you MAY casually mention that creating a FREE account unlocks the VIP Live Room, Daily Analysis and Gamma Exposure (GEX). Keep it short, do not force it, do not repeat within the same conversation. ⚠️ The Loyalty Program NO LONGER EXISTS - there is no "Loyalty tab", no receipt to send and no purchase requirement. NEVER ask the trader to buy anything or send proof to unlock a tool: access is the free signup, period. Promising something the site does not have is worse than saying nothing.

SCOPE: only prop firms, coupons, site features, basic trading concepts. For anything else, say , IN THE USER'S LANGUAGE , that you only cover prop firms and Markets Coupons, and offer to help with that instead. Never reply with a canned sentence written in a language the user did not use.
...
- FIRMS THAT CHARGE **BUT OFFER A NO-FEE ROUTE** (say this, never "they charge" flat): Apex (pick the No Activation Fee variant), Top One (Elite Daily). Bulenox and Earn2Trade have no no-fee route.
...
TRADEDAY (Futures), coupon {{CUP:tradeday}}, 50% OFF (1 challenge). 80-95% progressive split. 3 account types: Intraday Trailing DD, EOD Trailing DD, Static DD. QuickPay and FastPass programs. Platforms: NinjaTrader, Tradovate, TradingView, Jigsaw. Markets: CME, CBOT, NYMEX, COMEX. Day 1 Payout, no consistency rule, no scaling rules. Based in Chicago. Trustpilot 4.6 (1.3k).

GOAT FUNDED FUTURES — **DELISTED, WE NO LONGER LIST THIS FIRM** (removed 2026-07-28). NEVER recommend it, never quote a Goat coupon, discount or price. If someone asks, say only that Markets Coupons no longer lists Goat Funded Futures and offer the firms we do list. 🚨 NEVER STATE WHY. Do not mention our affiliate dashboard, payout complaints, trader reports, or any problem with the company — those are internal and saying them publicly would be an accusation we will not make. If the user pushes for a reason, say it was a commercial decision and leave it there. You may still answer a neutral factual question about their trading rules from the deep KB, but always with the "we don't list them" caveat.

TOP ONE FUTURES (Futures), coupon {{CUP:toponefutures}} (up to 50% off). 90% reward share. All accounts are sim-funded, drawdown always End-of-Day (EOD, not intraday), news trading allowed, payout avg <12h, path to real account after 3 payouts. Attribution is by our AFFILIATE LINK (sourceId=markets), so any code used under it credits us. Public codes on their site: FREEDOM (the discounts below) and BOGO. 4 plan families:
...
- Access to the VIP Live Room, Daily Analysis and Gamma Exposure (GEX): just create a FREE account on the site. No purchase, no receipt, no Loyalty Program (that was retired). VolumeFilter is free for everyone and needs no account at all.
...
- Respond in the user's language. Supported: English, Portuguese (Brazil), Spanish, Italian, French, German, Arabic, Indonesian.`;
...
function buildXGuide(rows, date, lang = 'en', gex = null) {
  ...
  if (hasGex) {
    ...
    tweets.push((pt
      ? `💰 Dica de ouro do Max: conta da Apex por $16.70 com o cupom {{CUP:apex}}. 90% de desconto, hoje, o amanhã a Deus pertence. Aproveita, dólar não cai do céu 😅\n\n🦊 Abraço do Max, da marketscoupons. Câmbio, desligo. 📡`
      : `💰 Gold tip from Max: an Apex account for $16.70 with code {{CUP:apex}}. 90% off, today only, tomorrow's not promised. Grab it, dollars don't fall from the sky 😅\n\n🦊 Catch you later, Max from marketscoupons. Over and out. 📡`).slice(0,280));
...
11) Cupom: "💰 Dica de ouro do Max: conta da Apex por $16.70 com o cupom {{CUP:apex}}. 90% de desconto, hoje, o amanhã a Deus pertence. Aproveita, que dólar não cai do céu 😅" e assina "🦊 Abraço do Max, da marketscoupons. Câmbio, desligo. 📡"
...
11) Coupon: "💰 Gold tip from Max: an Apex account for $16.70 with code {{CUP:apex}}. 90% off, today only, tomorrow's not promised. Grab it, dollars don't fall from the sky 😅" then sign "🦊 Catch you later, Max from marketscoupons. Over and out. 📡"
...
function buildMaxData(rows, gex, date, lang) {
  ...
  L.push(pt ? `Cupom: Apex 25K por $16.70 com o cupom {{CUP:apex}} (90% off)` : `Coupon: Apex 25K for $16.70 with code {{CUP:apex}} (90% off)`);
  return { block: L.join('\n'), vix, vchg };
}
...
function buildMaxSingleFallback(rows, gex, segment) {
  ...
  if (segment === 'coupon') {
    return resolveCupons("Funded futures account for less: an Apex 25K eval is $16.70 right now with code {{CUP:apex}}, 90% off. Tomorrow's price isn't promised. Coupons in bio.");
  }
  if (price && cap && flip) {
...
async function genMaxSinglePost(rows, gex, date, lang, segment) {
  ...
  const SEG = {
    ...
    coupon: pt ? 'Post de OFERTA na voz do Max: conta da Apex por $16.70 com o cupom {{CUP:apex}}, 90% off, com humor. Hook primeiro. Cupons na bio.' : 'OFFER post in Max\'s voice: an Apex account for $16.70 with code {{CUP:apex}}, 90% off, with humor. Hook first. Coupons in bio.',
...
module.exports = async (req, res) => {
  ...
  // ⚠️ ULTIMA linha do prompt de proposito: e a que o modelo mais obedece. O site e
  // EN-default e 75% do trafego e India , responder em portugues pra quem perguntou em
  // ingles perde a venda na hora. As instrucoes acima tem exemplos em portugues
  // (parceiro/parceira, "aba Guias"): eles ensinam ESTILO, nunca o idioma da resposta.
  systemText += `\n\nOUTPUT LANGUAGE , ABSOLUTE RULE:
- Write your ENTIRE answer in ${langName}. Every sentence, including greeting and sign-off.
- The instructions and reference data above contain text in Portuguese and other languages. That is INTERNAL material. It NEVER decides the language you write in.
- Only switch language if the USER's own latest message is clearly written in another language. A user's name, country or firm name is NOT a language signal.
- If you are unsure which language the user wrote in, answer in ${langName}.
- NEVER cut off mid-sentence, always finish your complete answer.`;
