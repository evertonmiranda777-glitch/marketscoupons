// E8 Markets: e8x.e8markets.com/affiliate
(async () => {
  try {
    const auto = await mcShouldE8('e8');
    if (!auto) return;
    await mcSyncE8({ auto: true });
  } catch (e) { console.warn('[MC] e8 auto-sync erro:', e); }
})();

async function mcShouldE8(firmId) {
...
async function mcMarkE8(firmId) {
  return new Promise(resolve => {
    chrome.storage.local.get(['mc_last_sync'], r => {
...
function mcScrapeE8Snapshot() {
  ...
  document.querySelectorAll('*').forEach(el => {
    ...
    const check = (labels, key) => {
      ...
      if (labels.some(l => lower === l || lower.startsWith(l))) {
        const nums = (pTxt.match(/US?\$?\s*[\d.,]+/g) || []).map(mcE8Num).filter(n => !isNaN(n));
        if (nums.length) snap[key] = nums[0];
      }
    };
    check(['paid:'], 'total_paid');
    check(['available to request:'], 'available_credit');
  });
  // Usage de cupons como "customers"
  let totalUsage = 0;
  document.querySelectorAll('*').forEach(el => {
    const t = (el.textContent || '').trim();
    const m = /Usage:\s*(\d+)/i.exec(t);
    if (m) totalUsage += +m[1];
  });
  if (totalUsage) snap.customers = totalUsage;
...
function mcScrapeE8Table() {
  ...
  document.querySelectorAll('table, [role="table"]').forEach(t => {
    ...
    t.querySelectorAll('tbody tr, [role="row"]').forEach(tr => {
      ...
      const d = mcE8ParseDate(tds[iDate]);
      if (!d) return;
      const commission = iCom >= 0 ? mcE8Num(tds[iCom]) : (iAmt >= 0 ? mcE8Num(tds[iAmt]) : 0);
      out.push({ date: d, commission, amount: iAmt >= 0 ? mcE8Num(tds[iAmt]) : commission });
    });
  });
  return out;
}
...
async function mcSendE8(payload) {
  try {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg && msg.action === 'sync_e8') {
    mcSyncE8({ auto: false }).then(sendResponse);
    return true;
  }
});
