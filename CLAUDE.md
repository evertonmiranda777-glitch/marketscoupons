...
**REGRA SEO páginas de firma = shells estáticos (LEI 15/jul):** as ~19 rotas de firma (`/apex`, `/bulenox`...) NÃO servem mais o `index.html` cru , servem **`firms/{id}.html`** (vercel.json rota `/(apex|...)` → `/firms/$1.html`). Cada arquivo é o **index.html byte-idêntico** com o `<head>` patchado por firma (title EN "Apex Coupon 90% OFF | MARKET", canonical próprio `/{id}`, description, OG/twitter, keywords + JSON-LD Product/Offer/AggregateRating/Breadcrumb). Antes o canonical apontava pra home → Google via as money-pages como duplicate → não rankeavam "{firm} coupon code". ⚠️ **MANUTENÇÃO CRÍTICA:** como o corpo é cópia do index.html, **toda vez que mexer no index.html, REBUILDAR:** `SUPABASE_ACCESS_TOKEN='sbp_...' node scripts/build-firm-seo-shells.mjs` (senão as firmas servem corpo velho). Templates EN vêm do `FIRM_SEO_META.en` (app.js ~1536); se mudar lá, refletir no `SEO` do script. `/firms/{id}.html` faz 308→`/{id}` (sem duplicata). Detalhe: [[project_sessao_2026_07_08]] §12.
