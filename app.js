...
async function loadFirmsFromSupabase() {
  try {
    ...
    if (error || !data || !data.length) {
      ...
      const cached = localStorage.getItem('mc_firms_cache_v3');
    ...
    try { localStorage.setItem('mc_firms_cache_v3', JSON.stringify(FIRMS)); } catch(e){}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if(FIRMS.length===0){
    try{ const cached=localStorage.getItem('mc_firms_cache_v3'); if(cached){ const arr=JSON.parse(cached); arr.forEach(f=>FIRMS.push(f)); } }catch(e){}
  }
