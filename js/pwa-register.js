...
(function () {
  ...
  async function subscribePush() {
    ...
    try {
      ...
      return { ok: false, reason: 'subscribe_failed', error: String((e === null || e === void 0 ? void 0 : e.message) || e) };
    }
    ...
    try {
      ...
      return { ok: false, reason: 'fetch_failed', error: String((e === null || e === void 0 ? void 0 : e.message) || e) };
    }
  ...
  // Religa a inscrição existente à conta do usuário que acabou de logar.
  // iOS PWA: o cara costuma ativar push DESLOGADO -> linha anonima (user_id null) ->
  // "enviar pra mim" (filtra user_id) nao acha o aparelho. Aqui, quando loga, re-envia
  // a subscription com o user_id; o servidor faz upsert por endpoint (merge) e linka.
  async function relinkPushUser() {
    try {
      const uid = window._currentUser && window._currentUser.id || null;
      if (!uid) return { ok: false, reason: 'no_user' };
      if (!_swReg) {try {_swReg = await navigator.serviceWorker.ready;} catch (_) {return { ok: false, reason: 'sw_not_ready' };}}
      const sub = await _swReg.pushManager.getSubscription();
      if (!sub) return { ok: false, reason: 'not_subscribed' };
      const key = sub.getKey ? sub.getKey('p256dh') : null;
      const auth = sub.getKey ? sub.getKey('auth') : null;
      if (!key || !auth) return { ok: false, reason: 'no_keys' };
      const r = await fetch('/api/push?action=subscribe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          endpoint: sub.endpoint,
          keys: { p256dh: arrayBufferToBase64Url(key), auth: arrayBufferToBase64Url(auth) },
          anon_id: function () {try {return localStorage.getItem('mc_anon') || null;} catch (_) {return null;}}(),
          user_id: uid,
          lang: function () {try {return localStorage.getItem('mc_lang') || 'en';} catch (_) {return 'en';}}()
        })
      ...
      return { ok: r.ok };
    } catch (e) {return { ok: false, reason: 'error', error: String((e === null || e === void 0 ? void 0 : e.message) || e) };}
  }
  ...
  window.mcTogglePush = async function () {
    ...
    if (window.MC_PWA.hasPushSubscription) {
      ...
      if (r.ok) alert(L.ok);else
      if (r.reason === 'permission_denied') alert(L.denied);else
      alert(L.error + ': ' + (r.reason || 'unknown'));
    }
    refreshPushButton();
  };

  document.addEventListener('mc-push-ready', refreshPushButton);
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', refreshPushButton);else
  setTimeout(refreshPushButton, 500);
