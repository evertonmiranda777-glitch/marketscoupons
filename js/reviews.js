...
(function (global) {
  ...
  function getAuthToken() {
    try {var _j$currentSession;
      const raw = localStorage.getItem('mc-user-auth');
      if (!raw) return null;
      const j = JSON.parse(raw);
      return j.access_token || ((_j$currentSession = j.currentSession) === null || _j$currentSession === void 0 ? void 0 : _j$currentSession.access_token) || null;
    } catch (e) {return null;}
  }

  function getCurrentUser() {
    try {var _j$currentSession2;
      const raw = localStorage.getItem('mc-user-auth');
      if (!raw) return null;
      const j = JSON.parse(raw);
      return j.user || ((_j$currentSession2 = j.currentSession) === null || _j$currentSession2 === void 0 ? void 0 : _j$currentSession2.user) || null;
    } catch (e) {return null;}
  }

  function esc(s) {return String(s !== null && s !== void 0 ? s : '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');}
  ...
  async function fetchStats(firmId) {
    ...
    return (data === null || data === void 0 ? void 0 : data[0]) || { total_reviews: 0, avg_rating: 0, count_5: 0, count_4: 0, count_3: 0, count_2: 0, count_1: 0 };
  }
  ...
  function bindForm(container, firmId, firmName, onSuccess) {
    ...
    if (submit) submit.addEventListener('click', async () => {
      ...
      msg.textContent = '';
      const rating = parseFloat(rate === null || rate === void 0 ? void 0 : rate.dataset.current) || 5;
