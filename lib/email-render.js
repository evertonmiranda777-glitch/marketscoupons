...
const INST_TEMPLATES = {
  ...
  'top3': {
    ...
    preheader: { pt:'Apex 100K $44,90 · Bulenox $23,65 · TakeProfit $165. Cupom no site.', en:'Apex 100K $44.90 · Bulenox $23.65 · TakeProfit $165. Coupon on the site.', es:'Apex 100K $44,90 · Bulenox $23,65 · TakeProfit $165. Cupón en el sitio.', fr:'Apex 100K $44,90 · Bulenox $23,65 · TakeProfit $165. Coupon sur le site.', de:'Apex 100K $44,90 · Bulenox $23,65 · TakeProfit $165. Gutschein auf der Seite.', it:'Apex 100K $44,90 · Bulenox $23,65 · TakeProfit $165. Coupon sul sito.', ar:'Apex 100K $44.90 · Bulenox $23.65 · TakeProfit $165. الكوبون على الموقع.' },
    ...
    firms: [
      { id:'apex',    name:'Apex Trader Funding · 100K', accent:'#F0B429', off:'90', price_full:'$449', price_disc:'$44,90', savings:'$404,10', coupon:'MARKET' },
    ...
    cta_url:       'https://www.marketscoupons.com/coupons?utm_source=email&utm_medium=campaign&utm_campaign=top3',
...
function buildTop3CardsHtml(t, lang, unsubUrl) {
  ...
  const cardsHtml = t.firms.map(f => `
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:14px;background:#fff;border:1px solid #e0e0e0;border-radius:12px;"><tr>
      <td width="64" style="padding:16px 0 16px 18px;vertical-align:middle;">
        <img src="https://www.marketscoupons.com/img/Firms/${f.id}.png" alt="${f.name}" width="48" height="48" style="display:block;background:#0a0d14;border-radius:10px;padding:6px;">
      </td>
      <td style="padding:16px 14px;vertical-align:middle;">
        <div style="font-family:${F};font-size:17px;font-weight:800;color:#1a1a1a;letter-spacing:-.3px;line-height:1.1;">${f.name}</div>
        <div style="margin-top:10px;font-size:13px;line-height:1;font-family:${F};">
          <span style="color:#9aa3b2;text-decoration:line-through;font-weight:600;">${f.price_full}</span>
          <span style="color:#9aa3b2;margin:0 6px;">&rarr;</span>
          <span style="color:${f.accent};font-weight:800;font-size:18px;">${f.price_disc}</span>
        </div>
        <div style="margin-top:6px;font-size:11px;color:#10B981;font-weight:700;letter-spacing:.3px;font-family:${F};">${SAV}${f.savings} &middot; ${CPN} ${f.coupon}</div>
      </td>
      <td width="64" align="right" style="padding:16px 18px 16px 0;vertical-align:middle;">
        <div style="font-family:${F};font-size:24px;font-weight:900;color:#DC2626;line-height:1;letter-spacing:-.5px;">&minus;${f.off}%</div>
      </td>
    </tr></table>`).join('');

  return `<!DOCTYPE html>
<html dir="${dir}" lang="${lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><meta name="color-scheme" content="light only">
<style>html,body{margin:0!important;padding:0!important;background:#f0f0f0!important;}table{border-spacing:0;border-collapse:collapse;}@media only screen and (max-width:600px){.ec{width:100%!important;}}</style></head>
<body style="margin:0;padding:0;background:#f0f0f0;font-family:${F};color:#1a1a1a;">
<div style="display:none;max-height:0;overflow:hidden;font-size:1px;color:#f0f0f0;opacity:0;mso-hide:all;">${pick(t.preheader, lang)}</div>
<table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#f0f0f0"><tr><td align="center" style="padding:32px 16px;">
<table class="ec" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e0e0e0;">

  <tr><td style="padding:26px 40px 22px;text-align:center;border-bottom:1px solid #f0f0f0;">
    <span style="font-size:22px;font-weight:800;color:#1a1a1a;letter-spacing:-.5px;">Markets <span style="color:${C};">Coupons</span></span>
  </td></tr>

  <tr><td height="4" bgcolor="#DC2626" style="background:#DC2626;font-size:0;line-height:0;">&nbsp;</td></tr>

  <tr><td bgcolor="#0a0d14" style="background:#0a0d14;padding:48px 40px 42px;text-align:center;">
    <div style="display:inline-block;padding:6px 14px;background:rgba(220,38,38,.14);border:1px solid rgba(220,38,38,.4);border-radius:99px;font-size:11px;font-weight:800;color:#fca5a5;letter-spacing:1.6px;text-transform:uppercase;margin-bottom:18px;">${pick(t.hero_pill, lang)}</div>
    <h1 style="margin:0 0 14px;font-size:32px;font-weight:800;color:#fff;line-height:1.2;letter-spacing:-.7px;">${pick(t.hero_h1, lang)}</h1>
    <p style="margin:0;font-size:13px;color:#b8c5d6;line-height:1.5;white-space:nowrap;">${pick(t.hero_sub, lang)}</p>
  </td></tr>

  <tr><td height="4" bgcolor="#DC2626" style="background:#DC2626;font-size:0;line-height:0;">&nbsp;</td></tr>

  <tr><td style="padding:32px 28px 18px;background:#fff;">
    ${cardsHtml}

    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:24px;background:#ecfdf5;border:1px solid #10B981;border-radius:14px;"><tr>
      <td style="padding:20px 22px;text-align:center;">
        ...
        <div style="font-size:36px;font-weight:900;color:#10B981;letter-spacing:-1px;line-height:1;">${t.total_value}</div>
        <div style="margin-top:6px;font-size:12px;color:#3a3f4a;">${pick(t.total_sub, lang)}</div>
      </td>
    </tr></table>

    <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center">
      <table cellpadding="0" cellspacing="0" border="0"><tr><td style="background:linear-gradient(135deg,${C},#D97706);border-radius:50px;">
        <a href="${t.cta_url}" style="display:inline-block;padding:16px 40px;font-size:15px;font-weight:800;color:#0a0a0a;text-decoration:none;letter-spacing:.3px;">${pick(t.cta, lang)}</a>
      </td></tr></table>
    </td></tr></table>

    <p style="margin:18px 0 0;font-size:11px;color:#9aa3b2;text-align:center;line-height:1.5;">${pick(t.disclaimer, lang)}</p>

    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:30px;border-top:1px solid #f0f0f0;"><tr valign="middle">
      <td width="44" style="padding-top:20px;"><table cellpadding="0" cellspacing="0" border="0"><tr><td style="background:linear-gradient(135deg,${C},#D97706);border-radius:50%;width:38px;height:38px;text-align:center;"><span style="font-size:16px;font-weight:800;color:#0a0a0a;line-height:38px;display:block;">L</span></td></tr></table></td>
      <td style="padding:20px 0 0 12px;"><div style="font-size:13px;font-weight:700;color:#1a1a1a;">Lara</div><div style="font-size:11px;color:#888;">${pick({pt:'Equipe Markets Coupons',en:'Markets Coupons Team',es:'Equipo Markets Coupons',fr:'Équipe Markets Coupons',de:'Markets Coupons Team',it:'Team Markets Coupons',ar:'فريق Markets Coupons'}, lang)}</div></td>
    </tr></table>
  </td></tr>

  <tr><td bgcolor="#f7f7f7" style="background:#f7f7f7;padding:22px 40px;text-align:center;border-top:1px solid #e0e0e0;">
    <p style="font-size:11px;color:#aaa;line-height:1.7;margin:0;">${pick(FOOTER_TEXT, lang)}<br><a href="${unsubUrl||'https://www.marketscoupons.com/'}" style="color:#aaa;text-decoration:underline;">${pick(UNSUB_TEXT, lang)}</a></p>
  </td></tr>
